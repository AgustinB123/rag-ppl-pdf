---
title: TECO: Esquema de seguridad
description: 
published: true
date: 2021-05-04T19:22:00.499Z
tags: 
editor: markdown
dateCreated: 2019-11-27T19:02:57.381Z
---

## Requerimientos:
* Autenticar las credenciales ingresadas en la app contra un dominio ([Active Directory](/instalacion/active-directory)).
* Una vez validado, recuperar los perfiles que comiencen con el prefijo GAPP_SIAF
* Las conexiones a la db deben ser a través de un único usuario.
* Las credenciales de la conexión de la conexión anterior deben recuperarse de un archivo, el cual posee la password encriptada.
* La aplicación FPA Console debe recuperar las credenciales de login de un archivo, el cual tmb debe tener la password encriptada. También debe contemplar el caso anterior.

## Propuesta FPA:

* Es requisito ingresar credenciales siempre que se inicia la app.  Las mismas pueden ser o no coincidentes con el usuario que inició en el puesto y se validan siempre contra el dominio especificado en la ventana de login. Si no se especifica un dominio, se utilizará el del usuario que inicio la app. Si la validación fue exitosa contra el dominio, se recuperan los grupos a los que pertenece, aceptando solo los que tienen prefijo “GAPP_SIAF”, para poder cargar los perfiles FPA que le corresponden usando la raíz del nombre para poder matchear con los códigos de la tabla PERFILES2.
...
* El prefjio para recuperar los perfiles del AD se definirá en el tag group_prefix del Config.Json del aplicativo correspondiente.
* Se entregará un aplicativo ([FPA Credentials](/core/fpa-credentials)) para que el cliente pueda encapsular las credenciales de acceso a la aplicación y a la base de datos encriptando las passwords.
* El aplicativo FPA Console debe utilizar los archivos generados por FPA Credentials para su ejecución y conexión con la base de datos.
...
1.	Instalar la aplicación [FPA Credentials](/core/fpa-credentials).
...
Antes que nada, necesitamos un server con AD ([Active Directory](/core/active-directory)) configurado. En él agregaremos las credenciales de los usuarios que queremos que pertenezcan a ese dominio. Una vez hecho esto, vamos a una máquina, que esté dentro de la misma red, nos logueamos con la cuenta admin local y establecemos el dominio creado para esa máquina. Para mas detalle de estos pasos, pueden seguir la siguiente guía https://win10faq.com/create-domain-windows-server/ .
...
Asumiendo que todo fue bien, el siguiente paso del sistema es recuperar los grupos principales a los que pertenece el usuario (solo tiene en cuenta los que tienen el prefijo establecido en el tag "group_prefix" del config.json, para este caso se estableció el valor "GAPP_SIAF_", si el mismo no existe o no fue establecido se emitirá un error indicándolo) para establecer los perfiles FPA. Si el usuario no tiene ningún grupo que corresponda a FPA se emitirá un error. Para buscar los perfiles que le corresponden al usuario se utilizan las raíces de los nombres de los grupos (es decir, se elimina el prefijo) y se busca en la tabla PERFILES los que tengan como código ese nombre. Si no se encuentra alguno de estos perfiles en la tabla se emitirá un error.
...
Para poder generar archivos de credenciales necesitamos tener instalada la app [FPA Credentials](/core/fpa-credentials), con ella vamos a poder generar 2 tipos de archivos credenciales:
...
## Como ejecutar los aplicativos sin AD

Para poder ejecutar la aplicacion sin necesidad de tener configurado un AD, necesitamos seguir los siguiente pasos:

* Establecer, en el config.json, el tag dev_ppl_mode en true. Con ello le decimos a la app, que en vez de autenticar contra el AD lo haga contra la maquina local, es decir que las credenciales que ingresemos en la pantalla de login se van a validar contra las cuentas locales que existan en la maquina.
* El usuario con que se ingresa a la app debe pertener al grupo **FPA_MODODESA**, con ello el usuario ingresara con perfil de superusuario.
